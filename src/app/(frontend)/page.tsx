...
import {
  ...
  File, Loader2, Camera, User, Building2, Phone, Mail, Hash,
  Check, ChevronDown, ChevronUp, X, LogOut, Sparkles
} from 'lucide-react';
...
export default function Home() {
  ...
  useEffect(() => {
    fetchProfile();
    fetchProperties();
    const logged = localStorage.getItem('mobile_logged_in');
    if (logged === 'true') {
      setIsLoggedIn(true);
    }
  ...
  if (!isLoggedIn) {
    return (
      <main className="flex flex-col min-h-screen bg-gradient-to-tr from-slate-900 via-slate-800 to-indigo-950 text-slate-100 font-sans items-center justify-center p-6 relative overflow-hidden">
        {/* Decorative Orbs */}
        ...
        <div className="w-full max-w-md bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 shadow-2xl flex flex-col gap-6 relative z-10">
          <div className="text-center">
            <div className="w-14 h-14 rounded-2xl bg-blue-500/20 text-blue-400 flex items-center justify-center mx-auto mb-4 border border-blue-400/25">
              <Sparkles className="w-7 h-7" />
            </div>
            <h2 className="text-2xl font-extrabold tracking-tight text-white mb-1">
              {language === 'zh' ? '巡检系统登录' : 'Inspection System Login'}
            </h2>
            <p className="text-xs text-slate-400">
              {language === 'zh' ? '请输入您的工作账号以启动房产巡检' : 'Please sign in to start your property inspection'}
            </p>
          </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-300 uppercase tracking-wider block">
                {language === 'zh' ? '用户名 / 邮箱' : 'Username / Email'}
              </label>
              <input 
                type="text" 
                placeholder="e.g. Zhu"
          ...
          <div className="text-center text-[10px] text-slate-500">
            Irish Property Inspection System &copy; 2026
          </div>
        </div>
      </main>
    );
    ...
    <main className="flex flex-col min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-200">
      ...
      {showResumeModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full space-y-6 animate-in fade-in zoom-in duration-200">
      ...
      {/* Header */}
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-lg border-b border-slate-200/60 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] font-medium text-slate-500 uppercase tracking-wider">{t.app.subtitle}</p>
          </div>
        </div>
        <button
          type="button"
          onClick={toggleLanguage}
          className="flex items-center justify-center w-11 h-11 bg-slate-100 rounded-full text-sm font-bold text-slate-700 hover:bg-slate-200 active:scale-90 transition-all shadow-sm"
        >
          {language === 'zh' ? 'EN' : '中'}
        </button>
      </header>
      ...
      <div className="flex-1 w-full max-w-md mx-auto p-6 flex flex-col gap-5 pt-6 pb-16">
        ...
        <div className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden p-5">
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">
              <span className="text-[10px] text-slate-400 font-bold uppercase block tracking-wider mb-0.5">{language === 'zh' ? '电话' : 'Phone'}</span>
              <span className="text-xs text-slate-700 font-semibold">{profile.phone || 'N/A'}</span>
            </div>
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100/50">
              <span className="text-[10px] text-slate-400 font-bold uppercase block tracking-wider mb-0.5">{language === 'zh' ? '邮箱' : 'Email'}</span>
              <span className="text-xs text-slate-700 font-semibold truncate block">{profile.email || 'N/A'}</span>
            </div>
            <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100/50 col-span-2">
              <span className="text-[10px] text-slate-400 font-bold uppercase block tracking-wider mb-0.5">{language === 'zh' ? '参考编号' : 'Reference No.'}</span>
        ...
        {activeTab === 'live' ? (
            ...
            <div className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 p-5 space-y-5 border border-slate-100">
                ...
                {selectedPropertyId && (
                  <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4 mt-2 space-y-2 text-xs text-blue-700 animate-in fade-in slide-in-from-top-1 duration-200">
                    <div className="flex justify-between items-center font-bold">
                      ...
                      <span className="font-mono">{selectedPropertyId.replace('prop_', '#')}</span>
                    </div>
                    {properties.find(p => p.id === selectedPropertyId) && (
                      <div className="grid grid-cols-2 gap-2 text-blue-600/80 pt-1">
                        ...
                        <div>🛏️ {properties.find(p => p.id === selectedPropertyId)?.rooms?.bedrooms || 0} Bed · 🛁 {properties.find(p => p.id === selectedPropertyId)?.rooms?.bathrooms || 0} Bath</div>
                ...
                {selectedPropertyId ? (
                  ...
                  coverPhotoDataUrl ? (
                    <div className="relative rounded-2xl overflow-hidden border border-slate-200 shadow-sm animate-in fade-in zoom-in duration-200">
                      <img src={coverPhotoDataUrl} alt="Property exterior" className="w-full h-44 object-cover" />
                      ...
                      <div className="absolute bottom-3 left-3 right-3 flex justify-between items-center">
                        <span className="text-white text-xs font-semibold bg-blue-600/85 backdrop-blur px-2.5 py-1.5 rounded-xl">
                  ...
                  coverPhotoDataUrl ? (
                    <div className="relative rounded-2xl overflow-hidden border border-slate-200">
                      <img src={coverPhotoDataUrl} alt="Property exterior" className="w-full h-40 object-cover" />
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  placeholder={t.setup.notes_placeholder}
                  value={notes}
                ...
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${pdfFile ? 'bg-blue-100' : 'bg-white shadow-sm'}`}>
              ...
              {/* Floorplan Upload */}
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">{t.setup.upload_floorplan}</label>
                <p className="text-xs text-slate-500">{t.setup.upload_desc}</p>
                ...
                >
                  {imageFiles.length > 0 ? (
                    <>
                      ...
                      <span className="text-sm font-medium text-green-700">{imageFiles.length} image(s) selected</span>
                      <span className="text-[11px] text-red-500 mt-1 font-bold"
                        onClick={e => { e.stopPropagation(); setImageFiles([]); }}>Remove All</span>
                    </>
                  ) : (
                    <>
                      <div className={`p-3 rounded-full mb-2 transition-colors ${isHoveringUpload ? 'bg-blue-100 text-blue-600' : 'bg-white text-slate-400 shadow-sm'}`}>
                        <UploadCloud className="w-5 h-5" />
                      </div>
                      <span className={`text-sm font-medium transition-colors ${isHoveringUpload ? 'text-blue-700' : 'text-slate-600'}`}>
                        Tap to select files
                      </span>
          ...
          <>
            ...
            <div className="bg-white rounded-3xl shadow-lg shadow-slate-200/50 p-5 space-y-5 border border-slate-100">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="hidden"
                  ref={videoInputRef}
                  onChange={e => { if (e.target.files?.[0]) setOfflineVideoFile(e.target.files[0]); }}
                />

                {offlineVideoFile ? (
                  <div className="p-4 bg-blue-50 border-2 border-blue-200 rounded-2xl flex items-center gap-3">
                  ...
                  >
                    ...
                    <span className="text-sm font-bold text-slate-700 group-hover:text-blue-600">{language === 'zh' ? '选择现场录像视频' : 'Select Local Video'}</span>
            ...
            {/* Error Message */}
            {offlineError && (
              <div className="p-4 bg-red-50 border border-red-200 text-red-700 text-sm font-semibold rounded-2xl text-center">
                ⚠️ {offlineError}
              </div>
            )}
      ...
      {offlineStatusStep > 0 && (
        <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-md flex flex-col items-center justify-center p-8 text-center text-white">
          <div className="bg-slate-800/80 border border-slate-700/50 rounded-3xl p-8 max-w-sm w-full space-y-6 flex flex-col items-center shadow-2xl animate-in fade-in zoom-in duration-200">
            {/* Circular Progress Ring */}
            <div className="relative w-24 h-24 flex items-center justify-center">
              <svg className="w-full h-full transform -rotate-90">
                <circle cx="48" cy="48" r="40" className="stroke-slate-700" strokeWidth="6" fill="transparent" />
                <circle cx="48" cy="48" r="40" className="stroke-blue-500 transition-all duration-300 ease-out" strokeWidth="6" fill="transparent"
            ...
            <div className="w-full bg-slate-700 h-1.5 rounded-full overflow-hidden">
              <div className="bg-gradient-to-r from-blue-500 to-indigo-500 h-full transition-all duration-300" style={{ width: `${offlineProgress}%` }} />
            </div>
          </div>
        </div>
      )}
      ...
      {showBackgroundModal && (
        <div className="fixed inset-0 z-[200] bg-slate-900/80 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-gradient-to-b from-slate-800 to-slate-900 border border-slate-700/50 rounded-3xl shadow-2xl p-8 max-w-sm w-full space-y-6 flex flex-col items-center animate-in fade-in zoom-in duration-200 text-white text-center">
