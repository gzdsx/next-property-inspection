...
export default function InspectionLive() {
  ...
  // ---------------------------------------------------------------------------
  const {
    isConnected, isConnecting, aiStatus, logs, records, uploadReportId, isUploading,
    startSession, stopSession, clearSessionData, captureVideoFrame, flagIssue, deleteRecord, updateRecord, uploadToServer
  } = useGeminiLive();
  ...
  return (
    <main className="relative w-full h-[100dvh] bg-black text-slate-100 font-sans overflow-hidden">
      ...
      {showResumePrompt && (
        <div className="absolute inset-0 z-[150] bg-black/85 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center">
          <div className="bg-slate-800/90 border border-slate-700/60 rounded-3xl p-7 max-w-sm w-full space-y-5 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            {/* Icon */}
            <div className="w-14 h-14 rounded-2xl bg-blue-500/20 border border-blue-400/30 flex items-center justify-center mx-auto">
      ...
      {flaggedPhoto && !showFlagFlash && (
        <div className="absolute top-16 left-3 right-3 z-[80] pointer-events-none">
          <div className="relative rounded-2xl overflow-hidden border-2 border-red-500 shadow-2xl shadow-red-500/40">
            <img
      ...
      <div className="absolute inset-0 z-0 bg-[#0d0d1a]">
        ...
        {!isConnected && !isConnecting && (
          <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-500 bg-[#0d0d1a]">
        ...
        {/* Header */}
        <header className="absolute top-0 left-0 right-0 flex items-center justify-between p-3 z-10 bg-gradient-to-b from-black/70 to-transparent">
          <button
            onClick={() => { stopSession(); router.push('/'); }}
          ...
          <div className="flex items-center gap-2 drop-shadow-md">
            <div className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-400 animate-pulse' : isConnecting ? 'bg-yellow-400 animate-pulse' : 'bg-slate-500'}`} />
        ...
        <div className="absolute top-16 left-2 right-2 flex flex-col gap-1.5 z-10 pointer-events-none">
          {records.slice(-2).map((rec) => (
            <div key={rec.id} className="bg-black/50 backdrop-blur-md border border-white/15 px-3 py-2 rounded-xl shadow-lg pointer-events-auto flex items-start justify-between">
              <div className="flex items-start gap-2 max-w-[70%]">
                ...
                {/* Status icon */}
                <span className="text-base shrink-0 mt-0.5" title={STATUS_CONFIG[getRecordStatus(rec.condition, rec.isManualFlag)].label}>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[45vh] flex flex-col bg-gradient-to-t from-black via-black/80 to-transparent z-10 overflow-hidden pt-12">
        ...
        {/* Controls */}
        <div className="flex-shrink-0 px-5 py-4 pb-[max(1rem,env(safe-area-inset-bottom))] flex justify-center items-center gap-4">
      ...
      {/* Record Detail Modal */}
      {viewingRecord && (
        <div className="absolute inset-0 z-[120] bg-black/90 backdrop-blur-sm flex flex-col pointer-events-auto animate-in fade-in zoom-in-95 duration-200">
          <header className="flex items-center justify-between p-4 border-b border-white/10 bg-black/50">
            ...
            <button onClick={() => setViewingRecord(null)} className="p-2 bg-white/10 rounded-full hover:bg-white/20">
              <X className="w-5 h-5" />
            </button>
          </header>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {viewingRecord.photoBase64 ? (
              ...
              <div className="w-full aspect-video bg-white/5 rounded-2xl border border-white/10 flex items-center justify-center text-slate-500">
      ...
      {isReviewMode && (
        <div className="absolute inset-0 z-[105] bg-[#0d0d1a] overflow-y-auto pb-32">
          <header className="sticky top-0 bg-black/80 backdrop-blur-xl border-b border-white/10 p-4 z-10 flex justify-between items-center">
            <button onClick={() => setIsReviewing(false)} className="text-blue-400 hover:text-blue-300 flex items-center text-sm font-semibold">
            ...
            <button onClick={() => { if(confirm('Discard this inspection?')) { clearSessionData(); setIsReviewing(false); } }} className="text-slate-500 hover:text-red-500 p-1">
              <X className="w-5 h-5" />
            </button>
          </header>
          ...
          <div className="p-4 space-y-4">
            {records.map(rec => (
              ...
              >
                {rec.photoBase64 ? (
                  <img src={`data:image/jpeg;base64,${rec.photoBase64}`} className="w-24 h-24 object-cover rounded-xl shrink-0" />
                ) : (
                  <div className="w-24 h-24 bg-white/5 rounded-xl flex justify-center items-center"><VideoOff className="w-8 h-8 text-slate-600"/></div>
                )}
      ...
      {(isUploading || uploadReportId) && (
        <div className="absolute inset-0 z-[110] bg-black/95 backdrop-blur-xl flex flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in-95 duration-300">
          {isUploading ? (
            ...
            <div className="flex flex-col items-center gap-6 bg-white/5 p-8 rounded-3xl border border-white/10 shadow-2xl">
              ...
              <div className="bg-white p-4 rounded-2xl shadow-xl">
                <QRCodeSVG value={`http://${typeof window !== 'undefined' ? window.location.hostname : 'localhost'}:3000/report/${uploadReportId}`} size={200} />
              </div>
              
              <div className="mt-2 w-full bg-black/50 rounded-xl p-3 border border-white/10 break-all text-left">
