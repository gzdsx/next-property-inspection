    ...
    const fetchReport = async () => {
      try {
        ...
        try {
          ...
          if (metaRes.ok) {
            fetchedMeta = await metaRes.json();
          }
        } catch (e) {}

        // Fallback/Inherit cover photo from allocated property if not present in meta
        try {
          const propsRes = await fetch('/api/properties');
          if (propsRes.ok) {
            const properties = await propsRes.json();
            const linkedProperty = properties.find((p: any) => 
              (p.drafts || []).some((d: any) => d.reportId === id)
            );
            if (linkedProperty) {
              if (!fetchedMeta) {
                fetchedMeta = {
                  address: linkedProperty.name,
                  inspectorName: "AI System",
                };
              }
              // Only inherit if the report has no cover photo of its own!
              if (!fetchedMeta.coverPhoto && !fetchedMeta.coverPhotoBase64 && linkedProperty.image) {
                const img = linkedProperty.image;
                if (img.startsWith('data:')) {
                  // Extract clean base64 data for PDF directly
                  fetchedMeta.coverPhotoBase64 = img.split(',')[1] || img;
                } else {
                  fetchedMeta.coverPhoto = img;
                }
              }
            }
          }
        ...
        try {
          const signedRes = await fetch(`/uploads/${id}_signed.json`);
          if (signedRes.ok) {
            const signedData = await signedRes.json();
            if (signedData.signature) {
              setSignatureBase64(signedData.signature);
              setIsSigned(true);
            }
          }
        } catch (e) {}
    ...
    return (
      <div className="dashboard-layout" style={{ justifyContent: "center", alignItems: "center", flexDirection: "column", gap: "20px" }}>
        <div className="glass-panel" style={{ padding: "50px", textAlign: "center", border: "1px solid var(--danger)" }}>
          ...
          <Link href="/public" className="sheet-select" style={{ display: "inline-block", marginTop: "20px", textDecoration: "none" }}>Go back home</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="dashboard-layout">
      ...
      <aside className="sidebar">
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <Link href="/public" className="sidebar-logo" style={{ textDecoration: "none" }}>
            <Sparkles size={24} />
          </Link>
          <nav className="sidebar-menu">
            <Link href="/public" className="sidebar-item active" title="Dashboard">
              <Home size={22} />
            </Link>
            <div className="sidebar-item" title="Analytics" onClick={() => router.push("/")}>
              <BarChart3 size={22} />
            </div>
          </nav>
        </div>
        ...
        <div style={{ position: "relative" }}>
          <div className="sidebar-avatar" onClick={() => setIsSidebarOpen(!isSidebarOpen)}>
            {meta?.inspectorName?.[0] || "Z"}
          </div>
        </div>
      </aside>
      ...
      <div className="main-viewport" style={{ flexDirection: "row" }}>
        ...
        <section style={{ flex: 1.5, display: "flex", flexDirection: "column", padding: "24px", overflowY: "auto", borderRight: "1px solid var(--panel-border)" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <Link href="/public" style={{ display: "flex", alignItems: "center", gap: "8px", color: "var(--foreground)", textDecoration: "none", fontWeight: "bold", fontSize: "0.9rem" }}>
              <ChevronLeft size={20} />
              Back to Dashboard
            </Link>
            <div style={{ display: "flex", gap: "10px" }}>
              <button 
                ...
                style={{ 
                  ...
                  color: "var(--foreground)", cursor: "pointer", fontWeight: "bold", fontSize: "0.85rem"
                }}
              >
                <Share2 size={16} />
                Share as link
          ...
          <div className="timeline-track-container">
            ...
            {isEditingTimeline && (
              <div className="glass-panel" style={{ padding: "16px", marginTop: "8px", display: "flex", flexDirection: "column", gap: "10px" }}>
                <h4 style={{ fontSize: "0.8rem", fontWeight: "bold" }}>Adjust Room Segments Transitions (Seconds)</h4>
        ...
        <section style={{ flex: 1, display: "flex", flexDirection: "column", padding: "24px", overflowY: "auto", background: "rgba(10, 15, 26, 0.15)" }}>
          <div style={{ marginBottom: "20px" }}>
            <h2 style={{ fontSize: "1.25rem", fontWeight: "900", letterSpacing: "-0.4px" }}>Property Room Inspection</h2>
            <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>Click on any room to verify full defect listings and edit element details.</p>
          </div>
          ...
          <div style={{ borderTop: "1px solid var(--panel-border)", paddingTop: "20px", display: "flex", flexDirection: "column", gap: "10px" }}>
            ...
            <div style={{ display: "flex", gap: "12px", width: "100%" }}>
              <button
                ...
                style={{
                  ...
                  cursor: isSigned ? "default" : "pointer", display: "flex", alignItems: "center", justifyContent: "center",
