...
function drawCoverPage(doc: jsPDF, options: ReportOptions, totalPages: number) {
    ...
    doc.text(inspector.phone, ML, infoY);
    infoY += lineH;
    doc.text(inspector.email, ML, infoY);
    infoY += lineH;
    doc.text(`Property inspected by ${inspector.inspectorName}`, ML, infoY);
    infoY += lineH;
    doc.text(`Reference: ${inspector.reference}`, ML, infoY);
    infoY += lineH * 1.5;
    ...
    doc.setFontSize(FS_SUBTITLE / 0.352778);
    setColor(doc, C_MAIN);
    doc.text('Carried Out', ML, infoY);
    infoY += lineH;

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(FS_BODY / 0.352778);
    setColor(doc, C_SECONDARY);
    doc.text(date, ML, infoY);
    infoY += lineH * 1.5;
    ...
    doc.setFontSize(FS_SUBTITLE / 0.352778);
    setColor(doc, C_MAIN);
    doc.text('Property report created with', ML, infoY);
    infoY += lineH;
...
function drawDeclarationPage(
    ...
    doc.setFontSize(FS_BODY / 0.352778);
    setColor(doc, C_MAIN);
    doc.text(inspector.companyName, ML, y);
    y += lineH;
    doc.setFont('helvetica', 'normal');
    setColor(doc, C_SECONDARY);
    doc.text(inspector.inspectorName, ML, y);
    y += lineH;
    doc.text(inspector.phone, ML, y);
    y += lineH;
